...
if ($stmt = $conn->prepare($sql)) {
    ...
    if ($result->num_rows > 0) {
        // Fetch the first reservation details
        $row = $result->fetch_assoc();
        ...
        $checkQuery = "SELECT COUNT(*) AS count FROM reservations WHERE reservation_id = ?";
        if ($checkStmt = $conn->prepare($checkQuery)) {
            $checkStmt->bind_param("i", $reservation_id2);
            $checkStmt->execute();
            $checkResult = $checkStmt->get_result();
            $checkRow = $checkResult->fetch_assoc();

            if ($checkRow['count'] == 0) {
                // Redirect if reservation ID is not found
        ...
        // Redirect if no reservations match the criteria
        header("Location: find_gown.php");
        exit;
    }
} else {
...
// If reservation exists, your main logic for this file continues here
?>
                    ...
                    <table class="table align-items-center mb-0">
                      <thead>
                        <tr>
                          ...
                          <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Date to Pick
                          </th>
                        ...
                        <tr>
                          <td>
                            <div class="d-flex px-2 py-1">
                              <div>
                                <img src="<?php echo $main_image2; ?>" class="avatar avatar-sm me-3"
                          ...
                          <td class="align-middle text-center text-sm">
                            <span
                              class="badge badge-sm bg-gradient-success"><?php echo htmlspecialchars($end_date2); ?></span>
                          </td>

                          <td class="align-middle">
                            <a class="btn btn-warning btn-md"
                              href="my_reservation.php?reservation_id=<?php echo $row['reservation_id']; ?>"><i
